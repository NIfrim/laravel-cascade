...
namespace Nifrim\LaravelCascade\Database\Relations\Concerns;
...
trait InteractsWithPivotTable
{
    ...
    public function detach($ids = null, $touch = true)
    {
        if ($this->using && ! empty($ids) && empty($this->pivotWheres) && empty($this->pivotWhereIns)) {
            ...
            if ($this->withSoftDeletes) {
                ...
                $attributes = [
                    $this->deletedAt() => $fresh->getTimestampMs(),
                ];

                if ($this->hasPivotColumn($this->updatedAt())) {
                    $attributes[$this->updatedAt()] = $fresh->getTimestampMs();
                }
    ...
    public function restore($ids = null, $touch = true)
    {
        if ($this->using && ! empty($ids) && empty($this->pivotWheres) && empty($this->pivotWhereIns)) {
            ...
            if ($this->withSoftDeletes) {
                $fresh = now();
                $attributes = [
                    $this->deletedAt() => Model::END_OF_TIME,
                ];
    ...
    public function newPivotQuery()
    {
        ...
        $query->when($this->withSoftDeletes, function (EloquentQueryBuilder $query) {
            $query->where($this->getQualifiedColumnName($this->deletedAt()), '=', Model::END_OF_TIME);
        });
        return $query;
    }
}
